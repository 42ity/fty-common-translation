/*  =========================================================================
    fty_common_translation_selftest.c - run selftests
...
// Tests for draft public classes:
    { "fty_common_translation_base", fty_common_translation_base_test, false, true, NULL },
...
{
    test_item_t *item;
    printf ("Running fty-common-translation selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_COMMON_TRANSLATION_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_common_translation_private_selftest (verbose, item->subtest);
#endif // FTY_COMMON_TRANSLATION_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_common_translation_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running fty-common-translation test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef FTY_COMMON_TRANSLATION_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_common_translation_private_selftest (verbose, test->subtest);
#endif // FTY_COMMON_TRANSLATION_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
